...
class ThinPoolImpl extends PoolImpl {
  ...
  _init(params) {
    if (!params.homogeneous) {
      errors.throwNotImplemented('Heterogeneous Pooling');
    }
    ...
    this._password = new ObfuscatedValue();
    this._walletPassword = new ObfuscatedValue();
    this._walletContent = new ObfuscatedValue();
    this._token = new ObfuscatedValue();
    this._privateKey = new ObfuscatedValue();
    ...
    if (params.password !== undefined) {
      this._password.set(params.password);
      this._userConfig.password = null;
    }
    // wallet password obfuscation
    if (params.walletPassword !== undefined) {
      this._walletPassword.set(params.walletPassword);
      this._userConfig.walletPassword = null;
    }
    // wallet content obfuscation
    if (params.walletContent !== undefined) {
      this._walletContent.set(params.walletContent);
      this._userConfig.walletContent = null;
    }
    // token obfuscation
    ...
    if (params.privateKey !== undefined) {
      this._privateKey.set(params.privateKey);
  ...
  async create(params) {
    this._init(params);
    this._eventCallback = params.poolEventReg;
  ...
  async _getConnAttrs() {
        ...
        if (typeof this._accessTokenFn === 'function') {
          ...
          if (typeof accessToken === 'string') {
            ...
            if (util.isTokenExpired(clonedAttrs.token)) {
              ...
              this._token.set(clonedAttrs.token);
            }
            ...
            if (util.isTokenExpired(clonedAttrs.token)) {
              ...
              this._token.set(clonedAttrs.token);
              this._privateKey.set(clonedAttrs.privateKey);
            }
          }
        } else {
          errors.throwErr(errors.ERR_TOKEN_HAS_EXPIRED);
    ...
    if (clonedAttrs.privateKey === null) {
      clonedAttrs.privateKey = this._privateKey.get();
    }
    return clonedAttrs;
  }
  ...
  async _destroy(connection) {
    try {
      if (connection.nscon.ntAdapter.connected) {
        ...
        // emit pool event when a connection is destroyed
        this._eventCallback("onPoolShrink");
      }
    } catch {
      return;
    }
  ...
  async close() {
    ...
    // clear _eventCallback to break the reference cycle created by the closure
    // capturing the user pool object.
  ...
  async bgThreadFunc() {
    ...
    while (!this._poolCloseWaiter) {
      ...
      let numToCreate = this._getNumConnsToCreate();
      ...
      for (let i = 0; i < numToCreate; i++) {
        try {
          ...
          // emit pool event when new connections are created in background
          this._eventCallback("onPoolExpand");
        } catch (err) {
          this._bgErr = err;
        }
      ...
      numToCreate = this._getNumConnsToCreate();
      // if no pending requests, wait for pending requests to appear!
      if ((numToCreate <= 0 || this._bgErr) &&
  ...
  async acquire() {
    ...
    while (this._freeConnectionList.length > 0) {
      ...
      this._eventCallback("onPoolConnectionHit");
      return conn;
    ...
    const conn = this._freeConnectionList.pop();
    this._eventCallback("onPoolConnectionMiss");
    this._usedConnectionList.add(conn);
    return conn;
  }
